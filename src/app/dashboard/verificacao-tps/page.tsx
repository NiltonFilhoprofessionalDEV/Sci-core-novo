...
interface VerificacaoTpsRegistro {
  id: string
  base_id: string | null
  data: string | null
  equipe: string | null
  tp_conforme: number | null
  tp_verificado: number | null
...
export default function VerificacaoTpsDashboard() {
  const { user } = useAuth()
  ...
  const additionalFilters = useMemo(() => {
    return (query: any) => {
      ...
      if ((isBace || isGerenteSecao) && user?.profile?.secao_id) {
        query = query.eq('base_id', user.profile.secao_id)
      }
  ...
  const { 
    ...
    refetch,
    isReady 
  } = useDashboardData<VerificacaoTpsRegistro>({
    tableName: 'verificacao_tps',
    selectFields: '*',
    orderBy: { column: 'data', ascending: false },
    limit: 1000,
    cacheKey: `verificacao-tps-${selectedBase || 'all'}-${selectedEquipes.join(',')}-${selectedMes || 'all'}`,
    additionalFilters
  })
  ...
  const totalConformes = registros.reduce((acc, r) => acc + (r.tp_conforme ?? 0), 0)
  const totalVerificados = registros.reduce((acc, r) => acc + (r.tp_verificado ?? 0), 0)

  const porEquipe = useMemo(() => {
    const map = new Map<string, { conforme: number; verificado: number; total: number }>()
    registros.forEach((r) => {
      const equipe = r.equipe || 'Sem equipe'
      if (!map.has(equipe)) {
        map.set(equipe, { conforme: 0, verificado: 0, total: 0 })
      }
      const entry = map.get(equipe)!
      entry.conforme += r.tp_conforme ?? 0
      entry.verificado += r.tp_verificado ?? 0
      entry.total += r.tp_total ?? 0
    })
    return Array.from(map.entries())
      .map(([equipe, { conforme, verificado, total }]) => ({
        equipe,
        conforme,
        verificado,
        total,
        percentualConforme: total > 0 ? Number(((conforme / total) * 100).toFixed(1)) : 0
      }))
  ...
  const porMes = useMemo(() => {
    ...
    registros.forEach((r) => {
      ...
      if (!map.has(key)) {
        map.set(key, { conforme: 0, verificado: 0, total: 0 })
      }
    ...
    return Array.from(map.entries())
      ...
      .map(([key, { conforme, verificado, total }]) => {
  ...
  const statusResumo = useMemo(() => {
    const conforme = totalTps > 0 ? Number(((totalConformes / totalTps) * 100).toFixed(1)) : 0
    const naoConforme = 100 - conforme
    return [
      { name: 'Conforme', value: conforme },
      { name: 'Não Conforme', value: naoConforme }
    ]
  }, [totalTps, totalConformes])
  ...
  const tabelaRegistros = useMemo(() => {
    return registros.map((registro) => ({
      ...
      dataFormatada: registro.data
        ...
        : '—',
      percentualConforme: (registro.tp_total ?? 0) > 0 
        ? Number((((registro.tp_conforme ?? 0) / (registro.tp_total ?? 1)) * 100).toFixed(1))
        : 0
    }))
  }, [registros])
  ...
  return (
    <div className="space-y-6 pb-12">
      <div>
        <h2 className="text-2xl font-bold text-[#1f1f1f]">Verificação de TPS</h2>
        <p className="text-sm text-[#1f1f1f]/70 mt-1">
          Monitoramento de conformidade dos equipamentos de proteção individual (TPS).
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-orange-200 bg-orange-50/40 p-10 text-center text-[#7a5b3e]">
          <p className="text-lg font-semibold text-primary">Ainda não há registros de verificação de TPS.</p>
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-4 mb-6">
            <div className="rounded-xl bg-gradient-to-br from-[#fb923c] to-[#f97316] p-6 text-white shadow-lg">
              ...
              <p className="mt-1 text-xs text-white/80">Verificações registradas</p>
            </div>
            <div className="rounded-xl bg-gradient-to-br from-[#fb923c] to-[#f97316] p-6 text-white shadow-lg">
              <p className="text-sm font-medium uppercase tracking-wide text-white/90">Total de TPS</p>
              <p className="mt-2 text-4xl font-black">{totalTps}</p>
              <p className="mt-1 text-xs text-white/80">Equipamentos totais</p>
            </div>
            <div className="rounded-xl bg-gradient-to-br from-[#fb923c] to-[#f97316] p-6 text-white shadow-lg">
              <p className="text-sm font-medium uppercase tracking-wide text-white/90">TPS Conformes</p>
              <p className="mt-2 text-4xl font-black">{totalConformes}</p>
              <p className="mt-1 text-xs text-white/80">{totalTps > 0 ? ((totalConformes / totalTps) * 100).toFixed(1) : 0}% do total</p>
            </div>
            <div className="rounded-xl bg-gradient-to-br from-[#fb923c] to-[#f97316] p-6 text-white shadow-lg">
              <p className="text-sm font-medium uppercase tracking-wide text-white/90">TPS Verificados</p>
              <p className="mt-2 text-4xl font-black">{totalVerificados}</p>
              <p className="mt-1 text-xs text-white/80">{totalTps > 0 ? ((totalVerificados / totalTps) * 100).toFixed(1) : 0}% do total</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
            <div className="xl:col-span-2 rounded-2xl border border-orange-200 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-lg font-semibold text-[#1f1f1f]">Conformidade por Equipe</h3>
                  <p className="text-xs text-[#1f1f1f]/60">TPS conformes, verificados e total por equipe</p>
                </div>
              </div>
              <div className="h-72 w-full">
                <ResponsiveContainer>
                  <BarChart data={porEquipe}>
                    ...
                    <Bar dataKey="conforme" fill="#fb923c" radius={[8, 8, 0, 0]} name="Conforme" />
                    <Bar dataKey="verificado" fill="#f97316" radius={[8, 8, 0, 0]} name="Verificado" />
                    <Bar dataKey="total" fill="#ea580c" radius={[8, 8, 0, 0]} name="Total" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-orange-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-[#1f1f1f]/60">Histórico de conformidade por mês</p>
              </div>
            </div>
            <div className="h-72 w-full">
              <ResponsiveContainer>
                <AreaChart data={porMes}>
                  <defs>
                    <linearGradient id="conformidadeEvolucao" x1="0" y1="0" x2="0" y2="1">
                  ...
                  />
                  <Legend wrapperStyle={{ color: '#1f1f1f' }} />
                  <Area type="monotone" dataKey="conforme" stroke="#fb923c" strokeWidth={3} fill="url(#conformidadeEvolucao)" name="Conforme" />
                  <Area type="monotone" dataKey="verificado" stroke="#f97316" strokeWidth={3} fill="url(#conformidadeEvolucao)" name="Verificado" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-border bg-card shadow-sm">
            <div className="border-b border-border px-6 py-4 bg-muted/30">
              <h3 className="text-lg font-semibold text-foreground">Detalhamento das Verificações</h3>
              ...
              <table className="min-w-full divide-y divide-border text-sm">
                <thead className="bg-muted text-xs uppercase tracking-wide text-foreground">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left font-semibold">TPS Conforme</th>
                    <th className="px-4 py-3 text-left font-semibold">TPS Verificado</th>
                    <th className="px-4 py-3 text-left font-semibold">TPS Total</th>
                    <th className="px-4 py-3 text-left font-semibold">% Conformidade</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border bg-card">
                  {paginatedRegistros.map((registro, index) => (
                    <tr key={`${registro.id}-${index}`} className="hover:bg-muted/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-foreground/80 font-medium">{registro.tp_conforme ?? '—'}</td>
                      <td className="px-4 py-3 text-foreground/80 font-medium">{registro.tp_verificado ?? '—'}</td>
                      <td className="px-4 py-3 text-foreground/80 font-medium">{registro.tp_total ?? '—'}</td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ${
                            registro.percentualConforme >= 80
                              ? 'bg-green-100 text-green-700'
                              : registro.percentualConforme >= 50
                              ? 'bg-yellow-100 text-yellow-700'
                              : 'bg-red-100 text-red-700'
                          }`}
                        >
                          {registro.percentualConforme.toFixed(1)}%
                        </span>
                      </td>
                    </tr>
                  ))}

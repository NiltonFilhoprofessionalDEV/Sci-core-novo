...
export function useDashboardMetrics(filters: DashboardFilters) {
  ...
  const meses = filters.meses && filters.meses > 0 ? filters.meses : 12
      ...
      try {
        const [
          ...
          (() => {
            ...
            if (filters.secaoId) query = query.eq('secao_id', filters.secaoId)
          ...
          (() => {
            ...
            if (filters.secaoId) query = query.eq('taf_registros.secao_id', filters.secaoId)
            if (filters.equipeId) query = query.eq('taf_registros.equipe_id', filters.equipeId)
            return query
          })(),
          (() => {
            let query = supabase
          ...
          (() => {
            let query = supabase
              ...
              .select('epi_entregue, epi_previsto, uniforme_entregue, uniforme_previsto, data, data_referencia, secao_id, equipe_id')
              .gte('data', startISO)
            if (filters.secaoId) query = query.eq('secao_id', filters.secaoId)
            if (filters.equipeId) query = query.eq('equipe_id', filters.equipeId)
            return query
        ...
        const monthMap = new Map(months.map((m) => [m.key, { month: m.label, aeronauticas: 0, naoAeronauticas: 0 }]))
        ...
        const tempoRespostaMap = new Map(months.map((m) => [m.key, { month: m.label, totalMinutos: 0, qtd: 0 }]))
        ...
        const tempoRespostaMedio = tempoRespostaSerie.reduce((acc, item) => acc + item.mediaMinutos, 0) / (tempoRespostaSerie.length || 1)
        ...
        const ptrBaProvasMap = new Map(months.map((m) => [m.key, { month: m.label, aprovados: 0, reprovados: 0 }]))
  ...
  }, [filters.secaoId, filters.equipeId, meses, refreshToken])

...
interface ControleTrocasRegistro {
  id: string
  nome_cidade: string | null
  nome_usuario: string | null
  equipe: string | null
  data_referencia: string | null
  quantidade_troca: number | null
...
export default function ControleTrocasDashboard() {
  const { user } = useAuth()
  ...
  const { 
    ...
    refetch,
    isReady 
  } = useDashboardData<ControleTrocasRegistro>({
    tableName: 'controle_trocas',
    selectFields: '*',
    orderBy: { column: 'data_referencia', ascending: false },
    limit: 1000,
    cacheKey: `controle-trocas-${selectedBase || 'all'}-${selectedEquipes.join(',')}-${selectedMes || 'all'}`,
    additionalFilters
  })
  ...
  const totalTrocas = registros.reduce((acc, r) => acc + (r.quantidade_troca ?? 0), 0)
  ...
  const porEquipe = useMemo(() => {
    ...
    registros.forEach((r) => {
      ...
      if (!map.has(equipe)) {
        map.set(equipe, { soma: 0, count: 0 })
      }
      const entry = map.get(equipe)!
      entry.soma += r.quantidade_troca ?? 0
      entry.count += 1
    })
    return Array.from(map.entries())
      .map(([equipe, { soma, count }]) => ({
        equipe,
        total: soma,
        media: count > 0 ? Number((soma / count).toFixed(1)) : 0
      }))
  ...
  return (
    <div className="space-y-6 pb-12">
      <div>
        <h2 className="text-2xl font-bold text-[#1f1f1f]">Controle de Trocas</h2>
        <p className="text-sm text-[#1f1f1f]/70 mt-1">
          Monitoramento de trocas de plantão e coberturas por equipe.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-orange-200 bg-orange-50/40 p-10 text-center text-[#7a5b3e]">
          <p className="text-lg font-semibold text-primary">Ainda não há registros de controle de trocas.</p>
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-6">
            ...
            <div className="rounded-xl bg-gradient-to-br from-[#fb923c] to-[#f97316] p-6 text-white shadow-lg">
              <p className="text-sm font-medium uppercase tracking-wide text-white/90">Total de trocas</p>
              <p className="mt-2 text-4xl font-black">{totalTrocas}</p>
              <p className="mt-1 text-xs text-white/80">Trocas realizadas</p>
            </div>
            <div className="rounded-xl bg-gradient-to-br from-[#fb923c] to-[#f97316] p-6 text-white shadow-lg">
              <p className="text-sm font-medium uppercase tracking-wide text-white/90">Média por registro</p>
              <p className="mt-2 text-4xl font-black">{totalRegistros > 0 ? (totalTrocas / totalRegistros).toFixed(1) : 0}</p>
              <p className="mt-1 text-xs text-white/80">Trocas médias</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
            <div className="rounded-2xl border border-orange-200 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-lg font-semibold text-[#1f1f1f]">Trocas por Equipe</h3>
                  <p className="text-xs text-[#1f1f1f]/60">Total de trocas realizadas por equipe</p>
                </div>
              </div>
              <div className="h-72 w-full">
                <ResponsiveContainer>
                  <BarChart data={porEquipe} layout="vertical" margin={{ top: 10, right: 30, left: 80, bottom: 10 }}>
                    ...
                    />
                    <Bar dataKey="total" fill="#fb923c" barSize={18} radius={[0, 4, 4, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-orange-200 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-[#1f1f1f]/60">Histórico de trocas por mês</p>
                </div>
              </div>
              <div className="h-72 w-full">
                <ResponsiveContainer>
                  <AreaChart data={porMes}>
                    <defs>
                      <linearGradient id="trocasEvolucao" x1="0" y1="0" x2="0" y2="1">
          ...
          <div className="rounded-2xl border border-border bg-card shadow-sm">
            <div className="border-b border-border px-6 py-4 bg-muted/30">
              <h3 className="text-lg font-semibold text-foreground">Detalhamento das Trocas</h3>
                ...
                <tbody className="divide-y divide-border bg-card">
                  {paginatedRegistros.map((registro, index) => (
                    <tr key={`${registro.id}-${index}`} className="hover:bg-muted/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-foreground/80 font-medium">{registro.quantidade_troca ?? '—'}</td>
                      <td className="px-4 py-3 text-foreground/80">{registro.nome_usuario ?? '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

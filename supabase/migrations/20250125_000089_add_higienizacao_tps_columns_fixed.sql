...
-- Adicionar constraints de validação (sem IF NOT EXISTS para constraints)
DO $$
BEGIN
    -- Adicionar constraint para tp_higienizado positivo
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_tp_higienizado_positive') THEN
        ALTER TABLE higienizacao_tps ADD CONSTRAINT check_tp_higienizado_positive CHECK (tp_higienizado >= 0);
    END IF;
    
    -- Adicionar constraint para tp_total positivo
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_tp_total_positive') THEN
        ALTER TABLE higienizacao_tps ADD CONSTRAINT check_tp_total_positive CHECK (tp_total > 0);
    END IF;
    
    -- Adicionar constraint de consistência
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_tp_higienizacao_consistency') THEN
        ALTER TABLE higienizacao_tps ADD CONSTRAINT check_tp_higienizacao_consistency CHECK (tp_higienizado <= tp_total);
    END IF;
    
    -- Adicionar constraint de unicidade
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'unique_higienizacao_tps_per_day') THEN
        ALTER TABLE higienizacao_tps ADD CONSTRAINT unique_higienizacao_tps_per_day UNIQUE(secao_id, data, equipe_id);
    END IF;
END $$;
